...
export default class App extends Component {
  ...
  getRequest = async () => {
    ...
    console.log(tickets);
    this.setState({ tickets, ticketsForRender: tickets.slice(0, 5), isLoading: false });
  }
  ...
  handleCheckbox = ({ target }) => {
    ...
    this.setState({ filters: { ...filters, ...newFilter }, ticketsForRender: filtered, buttonsState: null });
  }

  handleSort = ({ target }) => {
    const { tickets, filters, buttonsState } = this.state;
    if (buttonsState === target.id) {
      this.setState({ buttonsState: null, ticketsForRender: tickets.slice(0, 5) });
      return;
    }
    const newTickets = [...tickets];
    const sorted = target.id === 'cheapest' ? 
    ...
    this.setState({ ticketsForRender: filtered, buttonsState: newButtonsState });
  }

  render() {
    const { ticketsForRender, isLoading, isMenuHiden, filters, buttonsState } = this.state;
      ...
      <div id="container">
        ...
        <div className={mobileFilter}>
          ...
          <form className="checkboxes">
            <div className="checkboxField">
              <input id="all" type="checkbox" checked={filters.all} onChange={this.handleCheckbox} />
              <label htmlFor="all">Все</label>
            </div>
            <div className="checkboxField">
              <input id="without" type="checkbox" checked={filters.without} onChange={this.handleCheckbox} />
              <label htmlFor="without">Без пересадок</label>
            </div>
            <div className="checkboxField">
              <input id="oneTransfer" type="checkbox" checked={filters.oneTransfer} onChange={this.handleCheckbox} />
              <label htmlFor="oneTransfer">1 пересадка</label>
            </div>
            <div className="checkboxField">
              <input id="twoTransfers" type="checkbox" checked={filters.twoTransfers} onChange={this.handleCheckbox} />
              <label htmlFor="twoTransfers">2 пересадки</label>
            </div>
            <div className="checkboxField">
              <input id="threeTransfers" type="checkbox" checked={filters.threeTransfers} onChange={this.handleCheckbox} />
        ...
        <div className="ticketsBlock">
          <div className="buttons">
            <button id="cheapest" className={leftButton} onClick={this.handleSort}>Самый дешевый</button>
            <button id="fastest" className={rightButton} onClick={this.handleSort}>Самый быстрый</button>
          </div>
          <div className="tickets">
            {isLoading ? <div className="donut"></div> : ticketsForRender
